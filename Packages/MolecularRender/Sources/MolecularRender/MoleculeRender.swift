...
// Source - https://stackoverflow.com/a/54632872
// Posted by David Pasztor, modified by community. See post 'Timeline' for change history
...
public final class AdvancedRibbonBuilder {
  ...
  // MARK: - Mesh Builders
  private func buildHelixRibbon(_ residues: [ResidueInfo], colorMode: ColorMode) -> ModelEntity {
    let pts = residues.map { $0.caPosition }
    let spline = catmullRom(points: pts)
    let mesh = ribbonExtrude(path: spline,
                             width: 1.1,
                             thickness: 0.25)
    let mat : SimpleMaterial = material(for: .helix, mode: colorMode)
  ...
  private func buildSheetRibbon(_ residues: [ResidueInfo], colorMode: ColorMode) -> Entity {
    ...
    let mesh = ribbonExtrude(path: spline,
                             ...
                             thickness: 0.15,
                             arrowHead: true)
    let mat : SimpleMaterial = material(for: .sheet, mode: colorMode)
    let entity = ModelEntity(mesh: mesh, materials: [mat])
    return entity
  }
  private func buildCoilTube(_ residues: [ResidueInfo], colorMode: ColorMode) -> Entity {
    ...
    let mat : SimpleMaterial = material(for: .coil, mode: colorMode)
    let entity = ModelEntity(mesh: mesh, materials: [mat])
    return entity
  }
  ...
  private func tubeMesh(path: [SIMD3<Float>],
    ...
    for i in 0..<count {
      ...
      for s in 0..<radialSegments {
        ...
        uvs.append(SIMD2<Float>(Float(s)/Float(radialSegments), Float(i)/Float(count)))
      }
    }
    for i in 0..<count - 1 {
  ...
  private func ribbonExtrude(path: [SIMD3<Float>],
    ...
    for i in 0..<n {
      ...
      let normalsSlice: [SIMD3<Float>] = [up, up, -up, -up]
      let u = Float(i)/Float(n)
      positions.append(contentsOf: [topLeft, topRight, bottomLeft, bottomRight])
      normals.append(contentsOf: normalsSlice)
      uvs.append(contentsOf: [SIMD2<Float>(0,u), SIMD2<Float>(1,u), SIMD2<Float>(0,u), SIMD2<Float>(1,u)])
    }
    for i in 0..<n - 1 {
      let a = UInt32(i * 4)
      let b = UInt32(i * 4 + 1)
